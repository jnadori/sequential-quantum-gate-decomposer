...
*/
void Optimization_Interface::solve_layer_optimization_problem_LBFGS( int num_of_parameters, Matrix_real& solution_guess) {
        ...
        long long value;
        if ( config.count("max_iteration_loops_lbfgs") > 0 ) {
            config["max_iteration_loops_lbfgs"].get_property( value );
        ...
        if ( config.count("max_inner_iterations_lbfgs") > 0 ) {
            config["max_inner_iterations_lbfgs"].get_property( max_inner_iterations_loc );         
        }
        ...
        for (long long idx=0; idx<iteration_loops_max; idx++) {
            ...
            LBFGS cLBFGS(optimization_problem_combined, this);
            double f = cLBFGS.Start_Optimization(solution_guess, max_inner_iterations_loc);
